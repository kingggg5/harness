...
14. Loop runtime: concurrent claims yield one lease; Project/Run/contract/source/revision/claim bindings fail closed; delivery dedupe and queue-one stay bounded; pass requires best plus every declared verifier receipt; current evidence drift is detected; accepted writing commits descend from the claim and stay inside `write_scope`; only the accepted commit seeds the next iteration; zero and positive resource ceilings, failure/no-progress stops, pause/cancel, event-chain tampering, and exact timeout recovery behave deterministically without launching or stopping a worker.
