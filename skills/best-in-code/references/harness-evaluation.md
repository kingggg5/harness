...
13. Loop engineering: level/trigger combinations are valid; deterministic verification is mandatory; shell-string/inline-code verifiers fail closed; run/iteration/time/token/cost/external-call/failure/no-progress/parallel budgets are bounded; dedupe and overlap are explicit; active writers bind a rollback commit; scheduled/proactive loops stop before consequential effects; missing scheduler/supervisor capability degrades to one interactive iteration.
