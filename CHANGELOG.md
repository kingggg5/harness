...
- A provider-neutral Loop Engineering contract and validator for turn, goal, scheduled, and proactive levels: trigger/dedupe/overlap rules, deterministic-first verification, fixed run/iteration/time/token/cost/external-call budgets, rollback binding, human gates, and safe one-iteration fallbacks when supervision is unavailable.
