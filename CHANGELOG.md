...
- An optional dependency-free `loop-run` supervisor ledger with immutable contract/Project/Run binding, scheduled-delivery dedupe and queue-one overlap, one ephemeral iteration lease, SHA-256 evidence and event receipts, host-reported usage stops, accepted Git lineage and write-scope containment, pause/cancel, and exact timeout recovery.
- Loop runtime integration tests covering concurrent claims, token secrecy, verifier/best requirements, evidence rotation, scheduled runs, zero resource ceilings, no-progress termination, scoped commits, dirty baselines, and event-chain tampering.
